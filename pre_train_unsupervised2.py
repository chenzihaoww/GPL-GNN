...
def run(proc_id, n_gpus, args, devices, data):
    ...
    model = SAGE(in_feats, args.num_hidden, args.num_hidden, n_classes, args.num_layers, F.relu, args.dropout,
    ...
    for epoch in range(args.num_epochs):
        ...
        for step, (input_nodes, pos_graph, neg_graph, blocks) in enumerate(dataloader):
            ...
            if step % args.log_every == 0:
                print(
                    'proc_id:[{}],Epoch {:05d} | Step {:05d} | Loss {:.4f} | Speed (samples/sec) {:.4f}|{:.4f} | Load {:.4f}| train {:.4f}'.format(
    ...
    # print(model)
    # file_path = './pre_data/'
    # if not os.path.exists(f'{file_path}' + args.dataset):
    #     os.makedirs(f'{file_path}' + args.dataset + '/')
    # th.save(model.state_dict(), file_path + args.dataset + '_model_' + args.file_id + '.pt')
    # m_state_dict = torch.load(file_path + args.dataset + '_model_' + args.file_id + '.pt')  ####
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--num-negs', type=int, default=3)  ##1
    argparser.add_argument("--seed", type=int, default=200, help="random seed")
    argparser.add_argument("--aggregator-type", type=str, default=a.aggregator_type)
    ...
    argparser.add_argument('--log-every', type=int, default=40)
